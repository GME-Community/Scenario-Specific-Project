...
using System.IO;
using UnityEditorAV.iOS.Xcode.PBX;

namespace UnityEditorAV.iOS.Xcode
{
    ...
    public class PBXProject
    {
        ...
        public void AddAttributeForFile(string targetGuid,string fileGuid,string attribute)
        {
            var buildFile = BuildFilesGetForSourceFile(targetGuid, fileGuid);
            if (buildFile == null)
                return;
            if (!buildFile.attributes.Contains(attribute))
                buildFile.attributes.Add(attribute);
        }

        public void removeAttributeForFile(string targetGuid,string fileGuid,string attribute)
        {
            var buildFile = BuildFilesGetForSourceFile(targetGuid, fileGuid);
            if (buildFile == null)
                return;
            buildFile.attributes.Remove(attribute);
            // remove from known tags if this was the last one
        }
        /// <summary>
        ...
        */
        ...
        public void AddRemoteTargetDependeny(string targetGuid,string remoteFileGuid,string projectPath,string remoteInfo)
        {
            string projectGuid = FindFileGuidByRealPath(projectPath);
            if (projectGuid == null)
                throw new Exception("No such project");

            var containerProxy = PBXContainerItemProxyData.Create(projectGuid, "1", remoteFileGuid, remoteInfo);
            containerItems.AddEntry(containerProxy);

            var targetDependency = PBXTargetDependencyData.CreateRemote(remoteInfo, containerProxy.guid);
            targetDependencies.AddEntry(targetDependency);

            nativeTargets[targetGuid].dependencies.AddGUID(targetDependency.guid);
        }

        public void AddDynamicFramework(string targetGuid,string frameworkGuid)
        {
            string copyFilesGuid = AddCopyFilesBuildPhase(targetGuid,"Copy Files","","10");
            AddFileToBuildSection(targetGuid, copyFilesGuid, frameworkGuid);

            AddAttributeForFile(targetGuid, frameworkGuid, "CodeSignOnCopy");
            AddAttributeForFile(targetGuid, frameworkGuid, "RemoveHeadersOnCopy");

            AddBuildProperty(targetGuid, "LD_RUNPATH_SEARCH_PATHS", "@executable_path/Frameworks");
        }

		public string FindReferenceFileGuid(string filePath)
		{
			foreach(KeyValuePair<string, PBXReferenceProxyData> refData in references.GetEntries())
			{
				if(refData.Value.path.Equals(filePath))
				{
					return refData.Value.guid;
				}
			}
			return null;
		}
