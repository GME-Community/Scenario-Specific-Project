...
namespace UnityEditorAV.iOS.Xcode.PBX
{
    ...
    internal class PBXBuildFileData : PBXObjectData
    {
                                                      ...
                                                      string compileFlags)
        {
            ...
            buildFile.attributes = new List<string>();
            return buildFile;
        }
        ...
        public override void UpdateProps()
        {
            ...
            if (attributes != null && attributes.Count > 0)
            {
                if (settings == null)
                    settings = m_Properties.CreateDict("settings");
                var tagsArray = settings.CreateArray("ATTRIBUTES");
                foreach (string tag in attributes)
    ...
    internal class PBXTargetDependencyData : PBXObjectData
    {
        ...
        public static PBXTargetDependencyData CreateRemote(string target, string targetProxy)
        {
            var res = new PBXTargetDependencyData();
            res.guid = PBXGUID.Generate();
            res.SetPropertyString("isa", "PBXTargetDependency");
            res.SetPropertyString("name", target);
            res.SetPropertyString("targetProxy", targetProxy);
            return res;
        }
    }
